function __print_pearl_functions_help() {
cat <<EOF
Additional PearlROM functions:
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Pearl model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/pearl-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.pearl.device | grep -q "$PEARL_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $PEARL_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    if (adb shell getprop ro.pearl.device | grep -q "$PEARL_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        pearlremote
        git push pearl HEAD:refs/heads/'$1'
    fi
    '
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.pearl.device | grep -q "$PEARL_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/pearl/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $PEARL_FIXUP_COMMON_OUT ]; then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/pearl/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
